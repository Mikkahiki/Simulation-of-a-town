...
GREEN_GROWTH_MULTIPLIER = 1.2

DIRTY_GROWTH_MULTIPLIER = 1.4
...
def sustainability_index(state):
    ...
    score += state["economy"] * 0.3

    score += state["public"] * 0.3

    score -= state["co2_tons"] * 0.2

    score -= state["temperature"] * 15

    score -= state["crisis_level"] * 10
